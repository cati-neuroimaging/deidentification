...
from deidentification.archive import is_archive_in, is_archive_out, pack, unpack
...
def anonymize(dicom_in, dicom_out,
    ...
    # Handle archives
    is_dicom_in_archive = is_archive_in(dicom_in)
    is_dicom_out_archive = is_archive_out(dicom_out)
...
def check_anonymize(dicom_in,
    ...
    if os.path.isfile(wip_dicom_in):
        try:
            ...
            if is_archive_in(dicom_in) and os.path.exists(wip_dicom_in):
                shutil.rmtree(wip_dicom_in)
